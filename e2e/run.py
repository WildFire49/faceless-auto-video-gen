...
What it proves, in order:
   ...
   13. the script step writes a 9-beat script with every broken rule shown
       and Gate C shut; a reviewer fixes each problem by editing at Gate C;
       every number in the final script is in a fact its beat cites
       (re-checked independently); a wrong year edited in is caught and keeps
       Gate C shut, and the fix reopens it (gate_c.py)
...
from harness import (
    ...
    Step,
    check,
    check_closed_gate_refuses_edits,
    check_page,
    open_dashboard,
    run_step_and_wait,
)
...
def check_review_log_records_it(client: Client, run: Run, video_id: str) -> str:
    ...
    for gate in ("GATE_A_FACTS", "GATE_B_REFERENCES", "GATE_C_SCRIPT"):
...
def write_artifact(run: Run, run_dir: Path) -> None:
    ...
    if run.script:
        (run_dir / "script.json").write_text(json.dumps(run.script, indent=2), encoding="utf-8")
    if run.review_log:
    ...
    summary = {
        ...
        "script_beats": len((run.script or {}).get("beats", [])),
        "script_attempts": int((run.script or {}).get("attempts", 0)),
        "script_title": (run.script or {}).get("chosenTitle", ""),
...
def _script_section(run: Run) -> list[str]:
    """The script as a reviewer reads it: every beat, what it cites."""
    script = run.script or {}
    ...
    lines = [
        ...
        "## The script (Gate C)",
        "",
        f"**{script.get('chosenTitle') or '(no title chosen)'}** -- valid after "
        f"{script.get('attempts', '?')} attempt(s).",
        "",
        "| # | Role | Date | Narration | Cites |",
        "|---|---|---|---|---|",
    ]
    for beat in beats:
        voice = str(beat.get("voice", "")).replace("|", r"\|")
        lines.append(
            f"| {beat.get('n')} | {beat.get('role', '')} | {beat.get('yearStamp', '')} | "
            f"{voice} | {', '.join(beat.get('factIds', []))} |"
        )
    return lines
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=900,
        help="seconds to allow for the script step (up to 3 model attempts)",
    )
    parser.add_argument(
        "--beats", type=int, default=9, help="beats per video (config: script.beats)"
    )
    parser.add_argument(
        "--skip-reverify",
        action="store_true",
    ...
    try:
        ...
        check(
            run,
            "Gate C refuses to open with no script",
            lambda: gate_c.check_closed_without_script(client, vid),
        )
        check(
            run,
            "the script step runs and reports progress",
            lambda: gate_c.check_script_runs(client, vid, args.script_timeout),
        )
        check(
            run,
            "the script arrives with every problem shown and Gate C shut",
            lambda: gate_c.check_script_written(client, run, vid, args.beats),
        )
        check(
            run,
            "a reviewer can fix every problem at Gate C",
            lambda: gate_c.check_reviewer_fixes(client, run, vid),
        )
        check(
            run,
            "every number in the script is in a fact its beat cites",
            lambda: gate_c.check_numbers_grounded(client, run, vid),
        )
        check(
            run,
            "a wrong year is caught and Gate C stays shut",
            lambda: gate_c.check_wrong_year_caught(client, run, vid),
        )
        check(run, "the fix is accepted", lambda: gate_c.check_fix_accepted(client, run, vid))
        check(run, "a title can be chosen", lambda: gate_c.check_title(client, run, vid))
        title = str((run.script or {}).get("chosenTitle", ""))
        look(
            "gate-c-open",
            "Gate C, waiting for a decision",
            f"/v/{vid}/script",
            title,
            "Approve Gate C",
        )
        check(run, "Gate C opens", lambda: gate_c.check_opens(client, vid))
        check(
            run,
            "an approved Gate C refuses edits",
            lambda: check_closed_gate_refuses_edits(
                client,
                service="ScriptsService",
                method="UpdateBeat",
                payload={**run.injected, "videoId": vid},
                gate="C",
            ),
        )
        look(
            ...
            f"/v/{vid}/script",
            "Script approved",
            hide=("Approve Gate C", "Or write your own"),
        )
        ...
        check(
            run,
            "every gate decision is in the review log",
